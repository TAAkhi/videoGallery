		...
		if (preg_match_all($regex, $row->text, $matches, PREG_PATTERN_ORDER) > 0){
			...
			foreach ($matches[0] as $key => $match){
				...
				foreach ($found as $key => $filename)
				{
					...
					$gallery[$key]->sourceVideoFilePath = $siteUrl.$srcVideoFolder.'/'.self::replaceWhiteSpace($filename);
					...
					// Output
					$plg_html .= '<a href="'. JURI::root(true).str_replace('//','/',$gallery[$key]->sourceVideoFilePath).'"><img src="images/video/Big_Buck_Bunny.jpg" alt="Big Buck Bunny, Copyright Blender Foundation"></a>';
